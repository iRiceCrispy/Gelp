...
// Create a new game
router.post('/', validateGame, asyncHandler(async (req, res) => {
  const {
    ownerId, title, description, image, url, downloadLink, releaseDate,
  } = req.body;
...
// Edit a game
router.put('/:id(\\d+)', validateGame, asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id);
  const {
    title, description, image, url, downloadLink, releaseDate, currentVersion,
  } = req.body;
...
router.post('/:gameId(\\d+)/reviews', validateRating, asyncHandler(async (req, res) => {
